		...
		while True:
			...
			if msg:
				...
				# se cliente estiver na lista
					...
					if message[0] == 'set-update':
						# client manda informações para o servidor
						for cli in self.clients:
							if(cli.info == client and len(message) > 1):
								cli.setUpdates(message[1])
								break


					elif message[0] == 'get-updates':
						# enviando total de atualizações que serão enviadas
						total = len(self.clients) - 1
						self.sock.sendto(encoder.encode({'total': total.__str__()}), client)

						for cli in self.clients:
							if cli.info == client:
								continue
							else:
								self.sock.sendto(encoder.encode(cli.getUpdates()), client)


					# get all clientes updates 
